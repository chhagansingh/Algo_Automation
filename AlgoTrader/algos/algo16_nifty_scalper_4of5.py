"""
AlgoTrader — Algo 16: NIFTY Scalper (4/5 Confluence)
Source: nifty_scalper-main (AngelOne SmartAPI production scalper)

Original Strategy:
  - 5-checklist confluence: VWAP, EMA 9/21, Supertrend(7,3), RSI(7), India VIX
  - Enter when 4 of 5 indicators align
  - Max 5 trades/day, EOD square-off
  - Target +2% / SL -5% on option premium

India Adaptation:
  - Intraday only (5m bars for backtest)
  - ATM CE when bullish, ATM PE when bearish
  - Black-Scholes with same-day expiry (TTE ≈ 1/2520)
  - Slippage 0.5% + ₹50/roundtrip brokerage modeled
  - Daily loss limit: 2%
"""
import uuid
import math
...
TX_COST_SINGLE   = 20    # Discount broker cost for high-volume intraday scalping
RISK_FREE_RATE   = 0.07
STRIKE_STEP      = 50
BASE_IV          = 0.13

# Scalper parameters
TARGET_PCT       = 0.02   # +2% on option premium
SL_PCT           = 0.05  # -5% on option premium
MAX_TRADES_DAY   = 5
SLIPPAGE_PCT     = 0.0    # negligible for liquid ATM options
MIN_ENTRY_PREM   = 5.0    # skip if option price < ₹5 (avoid deep OTM lottery)
...
def _compute_signals(df: pd.DataFrame, vix_series: pd.Series) -> pd.DataFrame:
    """Compute 5-indicator confluence signals."""
    sig_df = pd.DataFrame(index=df.index)
    vwap_line = _vwap(df)
    close = df["close"]

    # 1. VWAP: price above/below with 0.05% buffer
    ...
    # 5. India VIX: decline <=0.5% = bullish bias, rise >=1% = bearish bias
    sig_df["vix"] = 0
    ...
    sig_df["entry"] = np.where(bull >= 4, 1, np.where(bear >= 4, -1, 0))

    # Store individual signals for diagnostics
...
class Algo16NiftyScalper45(AlgoBase):
    algo_id = "algo16"
    name    = "NIFTY Scalper (4/5)"
    ...
    def run_backtest(self, start_date: str, end_date: str, interval: str = "5m") -> Dict[str, Any]:
        ...
        # Fetch daily India VIX for the same range
        ...
        for i in range(len(df) - 1):
            ...
            if active_pos is not None:
                pos = active_pos
                bar_idx = i + 1
                # Cannot exit on same bar as entry
                if bar_idx <= pos["entry_bar"]:
                    continue
                spot = float(df["close"].iloc[bar_idx])
                bars_held = bar_idx - pos["entry_bar"]
                # Same-day expiry (1 trading day for BS stability)
                ...
                if exit_prem >= pos["target_px"]:
                    exit_px = exit_prem  # use actual market price at exit
                    exit_r = "TARGET_EXIT"
            ...
            # Signal active → open position at next bar open
            ...
            log_signal_check(
                ...
                reason=f"4/5 indicators aligned {'bullish' if sig==1 else 'bearish'}",
    ...
    def get_live_signal(self, spot: float, indicators: Dict[str, float]) -> Dict[str, Any]:
        # indicators must contain: vwap_signal, ema_signal, supertrend_signal, rsi_signal, vix_signal
        signals = [
